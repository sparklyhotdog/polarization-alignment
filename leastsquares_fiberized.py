...
def calc_expected_counts(rotation_list):
    ...
    for i in range(num_rotations):
        ...
        counts[2 * i] = random.gauss(0.5 * N_H * (1 + (np.asmatrix([1, 0, 0]) * F * rotation_list[i] * T * np.asmatrix(np.asarray([[1], [0], [0]])))[0, 0]), sigma)
        counts[2 * i + 1] = random.gauss(0.5 * N_D * (1 + (np.asmatrix([1, 0, 0]) * F * rotation_list[i] * T * np.asmatrix(np.asarray([[0], [1], [0]])))[0, 0]), sigma)

    return counts, actual_x
...
def least_squares_fitting(data, cost_threshold=1000):
...
if __name__ == "__main__":
    for index in range(1):
        ...
        counts = measure(num_rotations, rotation_list)
        print(counts)
        result = least_squares_fitting(counts, cost_threshold=1000)
